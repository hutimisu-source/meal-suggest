...
// 文字列に数字が含まれるかをチェックする関数(input_cand,serch_dish内で使用)
int is_number_string(const char* str) {
	for (int i = 0; i < (int)strlen(str); i++) {
		if (isdigit((unsigned char)str[i])) return 1;  // 数字があれば 1 を返す
	}
	return 0;  // 全て数字以外なら 0 を返す
}
...
int serch_dish(int b_Foods[],int i_DishNum[]) {
	...
	if (fp == NULL) {
		...
		for (int i = 0;i < i_Loop;i++) {
			...
			if (is_number_string(gl_st_DI[i].str_DishName)) return -104;							//料理名のデータに数字が紛れ込んでいないか確認
...
int main(void) {
	...
	if (i_Code != 0) {
		...
		else if (i_Code == -104)printf("cooking.txt内で料理名が数値になっています\n");
		else if (i_Code == -106)printf("cooking.txt内で料理数が適切ではないです\n");
